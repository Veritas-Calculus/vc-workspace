...
struct Desktop: Decodable, Identifiable, Equatable {
    let vmid: Int
    let name: String
    let node: String
    let status: String
...
struct DesktopConnection: Decodable {
    ...
    let sessionPolicy: NativeSessionPolicy
    let gateway: NativeGatewayConnection?

    init(id: String, protocolName: String, host: String, port: Int, username: String,
         password: String, issuedAt: Date, expiresAt: Date, desktopID: String,
         sessionPolicy: NativeSessionPolicy, gateway: NativeGatewayConnection? = nil) {
        self.id = id; self.protocolName = protocolName; self.host = host; self.port = port
        self.username = username; self.password = password; self.issuedAt = issuedAt
        self.expiresAt = expiresAt; self.desktopID = desktopID
        self.sessionPolicy = sessionPolicy; self.gateway = gateway
    }

    enum CodingKeys: String, CodingKey {
        case id, host, port, username, password, gateway
...
struct NativeSessionPolicy: Decodable, Equatable {
    let version: Int
    let revision: Int64
    let clipboardRedirection: Bool
    let driveRedirection: Bool
    let managedBackground: Bool
    let hash: String

    enum CodingKeys: String, CodingKey {
        case version, revision, hash
        case clipboardRedirection = "clipboard_redirection"
        case driveRedirection = "drive_redirection"
        case managedBackground = "managed_background"
    }

    var hasValidSnapshotHash: Bool {
        guard version == 1, revision > 0 else { return false }
        let clipboard = clipboardRedirection ? "true" : "false"
        let drives = driveRedirection ? "true" : "false"
        let background = managedBackground ? "true" : "false"
        let canonical = "{\"version\":\(version),\"revision\":\(revision)," +
            "\"clipboard_redirection\":\(clipboard)," +
            "\"drive_redirection\":\(drives)," +
            "\"managed_background\":\(background)}"
...
enum APIClientError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .rejected("invalid_credentials", _, _): return "用户名或密码不正确"
...
struct APIClient {
    ...
    init(server: String, session: URLSession? = nil) throws {
        ...
        if let session {
            ...
            self.session = URLSession(configuration: Self.interactiveSessionConfiguration())
        }
    }

    static func interactiveSessionConfiguration() -> URLSessionConfiguration {
        let configuration = URLSessionConfiguration.ephemeral
        // Return offline errors to the user instead of hiding login/settings
        // behind a resource-length connectivity wait. Retries remain explicit.
    ...
    func system() async throws -> SystemState {
        try await request(path: "/api/v1/system", timeoutInterval: 8)
    }
    ...
    func createConnection(vmid: Int, token: String, desktopScaleFactor: Int) async throws -> DesktopConnection {
        try await request(
            path: "/api/v1/native/desktops/\(vmid)/connections",
            method: "POST",
            token: token,
            headers: ["X-VC-Workspace-Desktop-Scale": String(desktopScaleFactor),
                      "X-VC-Workspace-Transport": NativeGatewayConnection.subprotocolName],
            timeoutInterval: 80
        )
    }

    func releaseConnection(id: String, token: String) async throws {
        let _: EmptyResponse = try await request(path: "/api/v1/native/connections/\(id)", method: "DELETE", token: token)
    }
