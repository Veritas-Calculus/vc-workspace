...
- 2026-09-12（macos-gateway-check 回归）：`make macos-gateway-check` 失败，`GatewayInteropTests` 的 `bundledNativeBridgePinsTheGuestInsideWSS` 两个用例（`matchingPin` 真/假）共 8 条断言不通过。原生传输停在 `state=0`（connecting）直到 10 秒判定超时，`transport.error()` 为空字符串；Gateway fixture 回报 `requests=1 authorized=1 closed=1 bytes=0 guest_tls=false guest_error=true peer_closed=true`。fixture 的 `native_pin` 模式先 `io.ReadFull` 读取 4 字节 X.224 头，`bytes=0` 说明原生客户端从未向隧道写出首个协商请求，`guest_error` 只是随后测试关闭隧道的结果，不是独立故障。

  已排除的原因：不是本轮 `rg`→`grep` 改动，因为同样使用该 `.app` 的 `make macos-rdp-reactivation-check` 通过，且 `libVCWorkspaceRDP.dylib` 只有一条 `@loader_path`，`verify-app.sh` 与 `codesign --verify --deep --strict` 均通过。也不是 `freerdp-tcp-diagnostic.patch`，该补丁只增加 `WLog_WARN`，`goto fail` 加花括号后控制流不变。桥接在 `pinned` 分支按 `freerdp_get_io_callbacks` 复制既有回调后只覆盖 `TCPConnect`，并用 `F_DUPFD_CLOEXEC` 复制描述符，`releaseNativeDescriptor` 关闭 Swift 侧副本不影响原生侧；FreeRDP 3.31.0 的 `rdp_new` 会把 transport 默认回调完整复制进 `rdp->io`，因此复制基线不是空指针集合。根因尚未定位，需要继续排查 FreeRDP 连接线程为何在 `TCPConnect` 前后停住。

  影响：该门禁在 CI 的 `macos` 作业中，远端此前因 `rg` 在更早的 `build-app.sh` 步骤就失败，从未执行到这一步。文档记录 2026-09-07 曾以 59 个测试/2 个 suite 通过该命令，当前为 67 个测试/3 个 suite，尚未确认该用例当时是否在内。推送前应先定位此项，否则 `macos` 作业仍会失败。

- 2026-09-12（远端 CI 首次失败归因与本地门禁复核）：基线 8 个 commit 已在远端 `main`，GitHub Actions 首次运行的 `core`、`guest-agent-artifacts`（Linux/Windows）通过，`macos` 作业在 `./scripts/build-app.sh release` 退出 1，日志为 `native RDP build requires rg`。因为 GitHub macOS Runner 不预装 ripgrep，`build-native-rdp.sh` 与 `verify-app.sh` 已去掉 `rg` 依赖改用 `grep`：三处字面匹配用 `grep -F`，`minos` 检查用 `grep -E '^[[:space:]]+minos 14\.0$'`；管道改为 herestring，避免 `pipefail` 下 `grep -q` 提前退出使上游收到 SIGPIPE 而误判未匹配。本机以真实 FreeRDP 3.31.0/OpenSSL 3.5.8 完整重建 `.app` 并通过 `verify-app.sh`。另修复 `go vet ./...` 失败：`internal/pve/principal_test.go` 的表驱动用例按值复制含 `sync.Mutex` 的 `Client`，改为 `*Client`。复核本地可执行的 CI 门禁：`go build`、`go vet`、`go test ./...`、`cargo test --workspace`、`pnpm check/test/build`、`make iac-check`、`swift test`（67 项）全部通过。尚未验证的远端门禁是两个 `docker build`、Playwright 浏览器回归、`make macos-gateway-check`/`macos-rdp-reactivation-check` 与两个 Linux 容器作业。2026-09-03 起的工作已在本轮纳入版本控制（`b637a52`），此前远端 CI 结论只覆盖 2026-09-02 的基线；推送后需要重新以完整代码验证一次。补充验证本地可跑的其余门禁：`control-plane` 与 `session-gateway` 两个镜像构建通过，`make macos-rdp-reactivation-check` 通过。
