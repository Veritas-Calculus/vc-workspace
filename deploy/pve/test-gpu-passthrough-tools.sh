...
common_env=(
  "VC_VDI_SYSFS_ROOT=$sysfs_root"
  "VC_VDI_PROC_CMDLINE_FILE=$proc_cmdline"
  "VC_VDI_ETC_ROOT=$etc_root"
  "VC_VDI_QM_BIN=$bin_root/qm"
  "VC_VDI_PCT_BIN=$bin_root/pct"
  "VC_VDI_BACKUP_ROOT=$backup_root"
  "VC_VDI_OUT_OF_BAND_CONSOLE_CONFIRMED=true"
)
...
[[ ! -e "$etc_root/modprobe.d/vc-vdi-vfio.conf" ]]
[[ ! -e "$etc_root/default/grub.d/99-vc-vdi-iommu.cfg" ]]
...
rollback_fixture="$backup_root/vc-vdi-gpu-backup-20260902T000000Z"
...
sed 's#vc-vdi-vfio.conf#unexpected.conf#' "$rollback_fixture/manifest" >"$rollback_fixture/manifest.invalid"
